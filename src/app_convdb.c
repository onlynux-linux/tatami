...
static int conv_main(void *pctx, struct apk_ctx *ac, struct apk_string_array *args)
{
	...
	apk_tar_parse(
		apk_istream_from_file(root_fd, "var/lib/tatami/db/scripts.tar"),
		read_script, ctx, apk_ctx_get_id_cache(ac));

	read_triggers(ctx, apk_istream_from_file(root_fd, "var/lib/tatami/db/triggers"));

	convert_idb(ctx, apk_istream_from_file(root_fd, "var/lib/tatami/db/installed"));

	adb_wo_obj(&idb, ADBI_IDB_PACKAGES, &ctx->pkgs);
	adb_w_rootobj(&idb);

	r = adb_c_create(
		//apk_ostream_to_file(db->root_fd, "var/lib/tatami/db/installed.adb", 0644),
