#!/usr/bin/lua5.3
-- Generate bash completion for tatami(8) from scdoc manpages

local template_main = [==[
# tatami(8) completion                                         *- shell-script -*-
# Automatically generated by tatami-tools (src/genbash.lua). Do not modify.

# Helpers
__tatami_installed_pkgs() { tatami query --summarize name --installed "$1*" 2>/dev/null; }
__tatami_available_pkgs() { tatami query --summarize name --available "$1*" 2>/dev/null; }
...
complete -F _comp_cmd_tatami tatami
]==]
...
local completion_function = {
	available='COMPREPLY=($(compgen -W "$(__tatami_available_pkgs "${cur}")" -- ${cur}))',
	installed='COMPREPLY=($(compgen -W "$(__tatami_installed_pkgs "${cur}")" -- ${cur}))',
}
